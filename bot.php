			...
			if ($event['type'] == 'message' && $event['message']['type'] == 'text') {
				...
				if(isRegistered($userId)==1){
					...
					if (strpos($text, 'ลงทะเบียน-') !== false) {
						...
						$messages = [
										'type' => 'text',
										'text' => 'คุณยังไม่ได้ลงทะเบียน จึงยังไม่สามารถสอบถามข้อมูลได้ เพื่อการตอบคำถามที่ถูกต้องกรุณาลงทะเบียนก่อน โดยพิมพ์ข้อความ "ลงทะเบียน-รหัสพนักงาน" ส่งมาที่ผมเพื่อลงทะเบียนครับ ตัวอย่างเช่น ลงทะเบียน-7100000' . $text
									];
						replyToUser($replyToken,$messages,$access_token);
					}
